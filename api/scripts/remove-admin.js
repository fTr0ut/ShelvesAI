...
async function removeAdmin() {
  ...
  if (!email) {
    logger.error('Usage: node scripts/remove-admin.js <email>');
    logger.error('Example: node scripts/remove-admin.js admin@example.com');
    process.exit(1);
  }
  ...
  try {
    ...
    if (!user.is_admin) {
      logger.info(`User "${user.username}" (${user.email}) is not an admin.`);
      process.exit(0);
    }
